...
logger = get_logger("dashboard_analyzer")
...
class DashboardAnalyzer:
    ...
    def load_data(self, filepath: str) -> pd.DataFrame:
        ...
        # auto-parse date columns based on keyword matching in column names
    ...
    def recommend_kpis(self) -> dict:
        ...
        # analyze() must run first so column_roles are available for the KPI prompt
    ...
    def _get_llm_client(self):
        ...
        # don't recreate the client on every call — reuse once built
        ...
        if self.provider == "azure":
            ...
            if self.endpoint:
                # caller explicitly passed a base URL — use it as-is
                kw["base_url"] = self.endpoint
            elif "gemini" in self.provider.lower():
                # Gemini keys (AIzaSy...) must hit Google's OpenAI-compatible
                # endpoint, NOT api.openai.com — that's what caused the 401.
                # Without this, the OpenAI client defaults to its own URL and
                # rejects Google keys every time.
                kw["base_url"] = "https://generativelanguage.googleapis.com/v1beta/openai/"
                logger.info("Gemini provider detected — routing to Google's OpenAI-compatible endpoint.")
    ...
    def _call_llm(self, prompt: dict) -> dict:
        ...
        def _provider_call():
            ...
            try:
                ...
                # classify transient errors so the retry policy picks them up
            ...
            # track tokens and approximate cost for billing awareness
            if resp.usage:
        ...
        result = resilient_json_llm_call(
            ...
            # lambda stores/retrieves the parsed result keyed by prompt hash
    ...
    def _build_profile(self) -> str:
        ...
        # cache key based on shape + column names — avoids rebuilding on repeated calls
        ...
        # cap at 50 unique values so we don't send a 1000-row category list to the LLM
    ...
    def _fallback_overview(self) -> dict:
        ...
        for id_col in roles["identifiers"]:
            # strip common suffixes to get a clean entity name (e.g. "customer_id" → "customer")
            base = (id_col.lower()
    ...
    def _fallback_kpis(self) -> dict:
        ...
        for m in roles.get("measures", [])[:5]:
            # pick dollar format for revenue/sales-style columns
        ...
        # record count is always a useful headline KPI
        ...
        # --- comparative metrics (exclude geographic columns since those belong on maps) ---
        dims = [d for d in roles.get("dimensions", [])
                if d not in roles.get("geographic", [])][:10]
...
if __name__ == "__main__":
    ...
    output_filename = "logs/output_dashboard_analyzer.json"
    try:
