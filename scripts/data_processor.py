...
MIN_ROWS = 1
MAX_COLUMNS = 500
# columns with more unique values than this are treated as text, not categories
CATEGORICAL_CARDINALITY_THRESHOLD = 50
# average string length above this threshold → classify as free-form text
TEXT_AVG_LENGTH_THRESHOLD = 50
MISSING_RATIO_WARNING = 0.05
MISSING_RATIO_HIGH = 0.30
DUPLICATE_RATIO_WARNING = 0.05
# standard IQR fence multiplier — 1.5 is the classic Tukey choice
IQR_MULTIPLIER = 1.5
TOP_K_CATEGORIES = 10
PREVIEW_ROWS = 5
...
class DataProcessor:
        ...
        try:
            if ext == ".csv":
                try:
                    ...
                    # latin-1 handles most Windows-generated files with special characters
                    logger.warning(
    ...
    def validate(
        ...
        *,
        ...
        for col_name in df.columns:
            ...
            # convert numpy types to plain Python so the schema stays JSON-serializable
        ...
        # -- preview rows (replace NaN with None for JSON compatibility) --
        preview = (
    ...
    def assess_quality(
        ...
        # warn on any column that's more than 30% missing — those are risky to visualize
        for col_schema in schema.columns:
        ...
        for col_name in schema.numeric_columns:
            ...
            # boolean mask: True where value falls outside the fences
        ...
        for col_name in schema.numeric_columns:
            ...
            # True where original value exists but coercion failed meaning bad data
    ...
    def profile(
        ...
        for col_schema in schema.columns:
            ...
            # -- Numeric --
            if sem_type == "numeric":
                # errors="coerce" so stray strings don't crash describe()
    ...
    def _infer_semantic_type(self, series: pd.Series) -> ColumnSemanticType:
        ...
        # 3. Numeric (native) — check for 0/1 booleans stored as integers
        ...
        if series.dtype == object or pd.api.types.is_string_dtype(series):
            ...
            # 4a. Numeric coercion — if 80%+ parse as numbers, treat as numeric
    ...
    def _compute_quality_score(
        ...
        return round(max(score, 0.0), 2)  # clamp so score never goes negative
    ...
    def _safe_float(val: Any) -> Optional[float]:
        ...
        try:
            f = float(val)
            # np.inf and np.nan aren't valid JSON — return None for those
...
# run full module of Data Processor to see if everything works together.
# ------------------------------
