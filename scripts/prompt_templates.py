...
# Keeping it here means we only need to tweak the persona in one place.
SYSTEM_PROMPT = (
...
def dataset_overview_prompt(profile_summary: str) -> dict:
    ...
    """
    # The triple-quoted f-string below embeds the profile inline so the LLM
    # sees all context in one shot — no multi-turn back-and-forth needed
...
def kpi_recommendation_prompt(
    ...
    Returns:
        Prompt dict.
    """
    # Pulling individual role lists with .get() so the function doesn't crash
    # if a role key is missing from an earlier LLM response
...
def visualization_recommendation_prompt(
    ...
    Returns:
        Prompt dict.
    """
    # Passing the full kpis/metrics lists as-is so the LLM can reference
    # source columns and formulas when deciding which chart type fits best
...
def data_quality_assessment_prompt(quality_report: str) -> dict:
    ...
    Returns:
        Prompt dict.
    """
    # The LLM returns an overall score + per-column issue list so downstream
    # code can decide whether to proceed with dashboard generation or halt for cleaning
...
def business_narrative_prompt(
    ...
    Returns:
        Prompt dict.
    """
    # story_arc in the response is meant to guide the ordering of sheets
    # on the final dashboard — overview first, then drill-down, then outliers
...
def dashboard_layout_prompt(visualizations: list, dashboard_title: str) -> dict:
    ...
    Returns:
        Prompt dict.
    """
    # Only pass the titles to keep the prompt short — full viz specs would eat too many tokens
