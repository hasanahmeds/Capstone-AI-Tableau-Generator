...
# Literal types keep Pydantic validation strict — any value outside
# these sets will raise a ValidationError at runtime.
...
class ColumnSchema(BaseModel):
    # extra="allow" so downstream code can attach arbitrary metadata
    # (e.g. parsed_datetime_success_ratio) without the model rejecting it
...
class DatasetSchema(BaseModel):
    ...
    # auto-generate a UUID on creation so every dataset has a stable ID
    ...
    # pre-bucketed column name lists so consumers don't have to filter
...
class OutlierReport(BaseModel):
    ...
    # only IQR is implemented right now; Literal keeps the door open for future methods
    method: Literal["iqr"] = "iqr"
    by_column: Dict[str, int] = Field(default_factory=dict)
    # stores the computed fence bounds per column for transparency
...
class QualityReport(BaseModel):
    ...
    def score_non_negative(cls, v: Optional[float]) -> Optional[float]:
        # guard against negative scores from buggy scoring logic
        if v is None:
            return v
        if v < 0:
...
class ColumnProfile(BaseModel):
    # Only one of numeric/categorical/datetime/text will be populated
    # depending on the column's semantic_type — the rest stay None
    model_config = ConfigDict(extra="allow")
...
# ------------------------
# Sets of string values that should be interpreted as boolean True/False
...
def detect_semantic_type(
    ...
    # Datetime — only classify if 80%+ of values parse successfully
    ...
    # Text vs Categorical — text has many unique values or long average length
...
def iqr_outlier_count(
    ...
    # need at least 4 points to get meaningful quartiles
    if len(s) < 4:
        return 0, None, None
    ...
    iqr = q3 - q1
    # zero IQR means all values are the same — nothing is an outlier
    if iqr == 0:
        return 0, None, None
...
def build_dataset_schema(
    ...
    for col in df.columns:
        ...
        cs = ColumnSchema(
            ...
            **extra,   # any extra metadata from detect_semantic_type (e.g. success ratios)
        )
...
def build_quality_report(
    ...
    for c in ds.numeric_columns:
        ...
        # difference between coerced NaN count and original NaN count gives coercion failures
    ...
    # Quality score (0..100) — penalize for data issues, weighted by severity
    penalties = 0.0
    ...
    if ds.numeric_columns and len(df) > 0:
        # average outlier fraction across all numeric columns
        ...
        if semantic == "numeric":
            ...
            if len(numeric) > 0:
                cp.numeric = NumericProfile(
                    ...
                    # ddof=1 for sample std (not population std)
            ...
            # cast to str so mixed-type columns (e.g. True/False as objects) sort correctly
            ...
            if len(parsed) > 0:
                cp.datetime = DatetimeProfile(
                    ...
                    # .to_pydatetime() converts from pandas Timestamp to stdlib datetime
...
class WorksheetSpec(BaseModel):
    ...
    def clean_field_lists(cls, v: List[str]) -> List[str]:
        # strip whitespace and drop empty strings to avoid bad shelf references in XML
...
class WorkflowState(BaseModel):
    ...
    # DataFrame is stored as JSON (orient="split") to keep the state serializable
