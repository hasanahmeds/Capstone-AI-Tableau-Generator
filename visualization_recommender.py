...
MIN_ROWS_FOR_SCATTER = 30    # scatter needs enough points to mean anything
HIGH_CARDINALITY_LIMIT = 50  # above this, skip the column for grouping
MAX_VISUALS = 10             # added limit so the dashboard stays focused
...
class VisualizationRecommender:
    ...
    def recommend(
        ...
        # --- Rule 1: time series ---
        # datetime + numeric is the most common dashboard pattern
        if ds.datetime_columns and ds.numeric_columns:
            visuals.extend(
                self._rule_time_series(ds.datetime_columns, ds.numeric_columns, ds.categorical_columns)
            )

        # --- Rule 2: categorical breakdowns ---
        # category + numeric gives bar charts and maybe pie charts
        if ds.categorical_columns and ds.numeric_columns:
            visuals.extend(
                self._rule_categorical(ds.categorical_columns, ds.numeric_columns)
            )
        ...
        if ds.numeric_columns:
            visuals.extend(
                self._rule_distributions(ds.numeric_columns, ds.categorical_columns)
            )

        # --- Rule 4: numeric relationships ---
        # scatter plots and heatmaps for pairs of numeric columns
        if len(ds.numeric_columns) >= 2 and ds.row_count >= MIN_ROWS_FOR_SCATTER:
            visuals.extend(
                self._rule_numeric_relationships(ds.numeric_columns)
            )
    ...
    def _rule_categorical(
        ...
        for cat_col in cat_cols[:2]:
            ...
            # skip if cardinality is way too high
            if unique_count is not None and unique_count > HIGH_CARDINALITY_LIMIT:
                logger.info(
                    f"Skipping '{cat_col}' for bar chart — "
                    f"{unique_count} unique values is too many."
                )
                continue
    ...
    def _rule_numeric_relationships(
        ...
        # correlation heatmap when we have 3+ numeric columns
        if len(num_cols) >= 3:
            heatmap = VisualizationSpec(
                chart_type="heatmap",
                title="Correlation matrix",
                rationale=(
                    f"Pairwise correlation across {len(num_cols)} numeric columns. "
                    f"Helps spot which variables move together at a glance."
                ),
                confidence=0.60,
            )
    ...
    def _guess_aggregation(self, col_name: str) -> str:
        ...
        # if the profile says values are in [0, 1], it's probably a proportion
    ...
    def _pick_grouping_column(self, cat_cols: List[str]) -> Optional[str]:
        ...
        for col in cat_cols:
            ...
            if uniques is not None:
                ...
                # no info — give it a middling score
...
if __name__ == "__main__":
    ...
    import json
    import pandas as pd
    from schemas import detect_semantic_type, DatasetSchema, ColumnSchema
    from schemas import build_profile_report
    ...
    print(f"Results saved to recommendations_output.json")