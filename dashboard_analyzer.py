...
Usage:
    ...
    # if you want to use without LLM (pure rule-based fallback)
...
class DashboardAnalyzer:
    ...
    def _call_llm(self, prompt: dict) -> dict:
        """Call the LLM with retries + exponential back-off.
        ...
        raise RuntimeError(
            f"LLM failed after {self.max_retries} retries: {last_err}"
        )
    ...
    def _build_profile(self) -> str:
        ...
        import hashlib
        key = hashlib.md5(
            f"{self.df.shape}{list(self.df.columns)}".encode()
        ).hexdigest()
