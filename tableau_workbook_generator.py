...
# 
# Logger setup — For Future log usage
#

logger = logging.getLogger("tableau_generator")
...
class TableauWorkbookGenerator:
    ...
    def __init__(
        ...
        # The upstream schema detection can miss date columns when
        # dates are stored as strings in formats like DD/MM/YYYY.
        # We run a quick fallback scan here so the generator can
        # add time-series worksheets even if the recommender didn't.
    ...
    def _plan_worksheets(self) -> WorkbookSpec:
        ...
        for viz in self.dashboard_spec.visuals:
            ...
            if viz.chart_type == "pie":
                # Pie charts: dimension on color, measure on size (angle)
    ...
    def _build_datasource_xml(self, parent: ET.Element, ds_name: str) -> ET.Element:
        ...
        for i, col_schema in enumerate(self.dataset_schema.columns):
            ...
            # Remote-type mapping (Tableau internal codes):
    ...
    def _build_worksheet_xml(self, parent: ET.Element, ws: WorksheetSpec, datasource_name: str) -> ET.Element:
        ...
        ET.SubElement(pane_el, "mark").set("class", "Automatic")
    ...
    def _build_dashboard_xml(self, parent: ET.Element, title: str, worksheets: List[WorksheetSpec], width: int, height: int) -> ET.Element:
        ...
        The dashboard structure in Tableau is:
            <dashboard name="...">
              <style />
              <size ... />
              <zones>...</zones>
        ...
        Empty <datasources> and <datasource-dependencies> elements are
        NOT valid at the dashboard level and must be omitted.
        """
        ...
        # NOTE: Do NOT add empty <datasources> or <datasource-dependencies>
        # here — they are invalid without proper children at the dashboard level.

        # 3. Zones — embed each worksheet
    ...
    def _add_worksheet_zone(
        ...
        Worksheet zones in Tableau do NOT have a 'type' or 'type-v2'
        attribute.  They're identified by having a 'name' attribute that
        matches a worksheet name.  Tableau figures out the rest.
        """
    ...
    def _build_windows_xml(self, root: ET.Element, worksheets: List[WorksheetSpec]) -> None:
        """Build the <windows> section that Tableau requires to display sheets.

        This tells Tableau Desktop which sheet to show when the workbook
        is first opened. Without this, Tableau may crash or show nothing.

        Structure (from the working .twb):
        ...
        # Show the first worksheet maximized
    ...
    def export_twb(self, output_path: str) -> str:
        ...
        # ElementTree.write() doesn't produce a standardXML by default.
        # We manually indent the tree first so the output is human-readable and matches the formatting of Tableau-generated .twb files.
    ...
    _DATE_KEYWORDS = {
        ...
        "order_date", "ship_date", "order date", "ship date",
    }
    ...
    def _detect_date_columns_fallback(self) -> List[str]:
        ...
        The Superstore dataset, for instance, has "Order Date" and
        "Ship Date" stored as strings in DD/MM/YYYY format.  Pandas
        detect_semantic_type can miss these if the format is ambiguous.
...
if __name__ == "__main__":
    ...
    # python tableau_workbook_generator.py train.csv
    #----------------------
    if len(sys.argv) > 1:
